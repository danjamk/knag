...
interface. The editing caret is that same block (#228), drawn by the editor behind the
native caret because a native caret cannot be made wide. It is steady; the native caret,
painted the same amber, blinks inside it unseen. 🔴 **Never hide the native caret** — on
iOS `caret-color` is also the colour of the selection handles and the highlight, so
`transparent` there is a selection nobody can see or grab. Found on the phone.
